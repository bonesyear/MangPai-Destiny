...
- **当前成绩**（blind_eval rubric v8，**P2 官命 fp 窄修簇批 2026-09-18**）：
...
（P2 前：trainset 官 100✅/15❌ 86.96%。P2 +2 全预注册，M3 噪声带内、CI 下界 79.6%→81.6%）
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 1088 collected（**1087 passed+1 xfailed，P2 实测**；P1 1077+1xf、L1 1066+1xf、旧记 G2 838+1xf+19xp、批10 499 均作废）+ blind_eval 快照零翻转 + 双 seed 逐字节一致 + `scripts/check_layering.py`（分层单向）+ `scripts/check_typing_imports.py`（typing/import 面）（旧 853 口径 2026-07-17 起作废）。
...
| `python3 -m pytest mangpai/tests/ -q` | 1088 collected（**1087 passed+1 xfailed，P2 实测**；旧记 P1 1077、L1 1066、842/822(N2b+N3)、794、776、682、499、473 均作废；19 xp 死测试 H-fix-4a 随 chuangong 删除） |
...
python3 -m pytest mangpai/tests/ -q               # 1088 collected（1087 passed+1 xfailed，P2 实测）
...
- trainset 294：官 **88.70%**（13❌=§6.2，P2 后）/ 财 52.21%（❌10=§6.3）/ 职 47.06%（33❌=§6.1）。
...
- 三维攻坚正式收官；后续方向（若重启）：~~官命 fp 窄修簇 A12-A18~~（P2 已清 2+收档 5）与检测簇 A8/A11/A19（P1 已清）、财命 A13/A4/G5 残簇（P3 批对象）、职业中医/军警/lawyer 盲区（军警墓库做功=P4 批对象；均须新突破面，旧窄通道已尽）。

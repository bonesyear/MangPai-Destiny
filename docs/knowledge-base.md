...
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 1067 collected（**1066 passed+1 xfailed，L1 实测**；H-fix-4a 起 19 xp 死测试随 chuangong 删除；旧记 G2 838+1xf+19xp、G1 831、N2b+N3 822、缺口批2 794、修批E6/E7 776、修批B/C 682、批10 499 均作废）+ blind_eval 快照零翻转 + 双 seed 逐字节一致 + `scripts/check_layering.py`（分层单向）+ `scripts/check_typing_imports.py`（typing/import 面）（旧 853 口径 2026-07-17 起作废）。
...
| `python3 -m pytest mangpai/tests/ -q` | 1067 collected（**1066 passed+1 xfailed，L1 实测**；旧记 842/822(N2b+N3)、794、776、682、499、473 均作废；19 xp 死测试 H-fix-4a 随 chuangong 删除） |

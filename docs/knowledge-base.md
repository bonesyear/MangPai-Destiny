...
> 最后更新：2026-08-17（F0 知识库勘误批：十批审计 46 条勘误+批1-4 散见 8 条落盘，见 §10；引擎代码零改动）。引擎当前 HEAD 基线 = `snapshots/20260814_c.json`。
...
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 499（批10 collect 实测，F0 复核 499 collected；旧记 473 作废）+ blind_eval 快照零翻转 + 双 seed 逐字节一致（旧 853 口径 2026-07-17 起作废）。
...
- `classify_strength`：身强/身弱/中和/从强/从弱；从格四规则=根被坏三式（双夹冲/邻支刑穿/合会化入异党——印生身不坏）+一方成势闸（异党单五行主气≥3——⚠️批6 勘误：书无此数值阈值，系自造后验归纳，应知；化势宽口径 selfc<conc 可用合局计数，两停仍主气）+从旺从禄三道闸；印根计本/中气、日主根计余气两套并存（22期例1/2 vs 例5，批6 独立复核属实）。
...
- 财富看法：财星当财/禄神当财（八字无财时口径）/伤食当财/官杀当财两式（财统官/官统财，须 cai≥1 零财无可统）/制不尽当财/过河拆桥（富格 vs 破财分两键——⚠️批6 勘误：「ch14 与 caiming 同名相反两诀并存」**不成立**，ch14 gaoji:19772-19815 与中级 2976-2978 皆只有发财义，「制不尽=破财」分键系引擎自造非书诀，§6.4「已收口」定性同步修正）/取财五法（经营/风险/智力/体力/工薪）。
...
- ⚠️批6 补记：官禄格定义口径与书分歧（书 zhongji:3969=印生禄 vs 代码=官星坐禄）；李昌镐书内两造并存（lixiangxue:11202 vs 3576-3584），备案。
...
- performer：桃花栈（食伤+桃花+财俱现+4/桃花居日柱+2 等）**一律不动**（刘晓庆/li154 靠其过阈——⚠️批7 勘误：「靠其过阈」是拟合事实非书证，刘晓庆书锚=食神泄秀木火 gaoji:1610 非桃花，桃花栈本身无书锚）；无桃花通道=柱级食伤≥2+食伤做功+无桃花+无主气明财+3（乔布斯豁免闸）；金水声音+4（金日主+水食伤主气+食≥2柱+比劫≥3）；桃花让位-3（仅财入印墓宾位命中者）。
...
- 入墓=得到/控制/占有（做功语义）；开库=释放。changsheng 己墓戌 vs muku 己墓辰=中性层与盲派层未分离的已知分歧——⚠️批1 补记：分歧实为**三方**不一致，TOMB_MAP 缺「戌=土墓」（段氏五行墓库章明言「土墓在辰、戌」双位，表仅 `辰:[水,土]`），消费方 caiming/guanming/gongliang/laoyu（F2 批已批准解锁 constants 修）。
...
- 驿马=段氏三支皆马：每支映射所属局对冲三支（申子辰马在寅午戌…），首位=传统单点向后兼容；⚠️批7/8 勘误：zaihuo 车祸 `ma_count≥1` 非「近恒真」是**恒真**（任意盘 count≥3，随机 2000 盘 min=3），且 count 语义=映射对冲支并集数非在局马数（应消费 in_pillars）——「死判据+语义错」，原「已知行为」备案低估；另书锚「以年支日支为主」与实现「四柱皆起取并集」相左未记。
...
- **jiaoyun/起运（批9 升级 P0）**：交运年=出生年+起运岁把虚岁当实岁（书例 2005 生 3 虚岁起运，书口径 2007 引擎 2008，系统性晚一年）；起运岁三处叠加偏离书明文（理象学:3854-3856/3864-3877）：实岁小数 vs 整数虚岁「余一舍余二进一」/时刻精确天数 vs 书整日差/下游 int() 截断=「余二也舍」；「不足一天一岁起运」未实现。
- **晚子时（批9 新 P0）**：bazi_calc 两种 late_zi_method 均与理象学:3703-3716 书例不符（书=日柱本日+时柱推转一轮纳次日干），`calc_bazi_full` 未暴露该形参；凌晨早子时全对——「晚子时」排盘分界点从未正确实现。
- **anhe 子巳（批1 P1→批9 升级 P0）**：暗合表多「子巳」，初级:3218「只有三个」显式排除；传播五处修表须同步：zuogong_detect:529/liunian:115/yunfan:101/zeishen:374/prompts/mangpai.md:16。
...
- **virtual_solid 两原则偏离与书相反**（批9）：「虚实只就一柱干支而言」（理象学:5647-5649）被改成全局四支找根；「坐印都是实」（初级:2461，甲子列实表）被判虚透怕克；经 is_solid 传导 zuogong_confirm「虚透被克损害加重」标注；docstring「已知争议：根的范围」轻描淡写应重写。同型：wood_type 漏「水不生木之根也是死木」甄别（岳飞造子水不生卯根被误判活木，与书明例:3187-3189 相反）。
...
- **shenshu/shipaige 对照源仅郑民生公开碎片**（批8）：段氏五书 grep「一财是财/十排歌」零命中；shenshu 数量诀 70 句与碎片逐字吻合（干净）；shipaige 断语层 39 条碎片几乎零实现+两处冠名冲突（官杀为子/劫财抗杀），docstring「置信度：低」自承属实——**shipaige 断语层不可作书证**。
...
| ans12-下岗财会 | 局部最优实证：gongliang 任何降层反而更糟（真锁=caiming 富格 floor+开财库链）→⚠️批6 勘误：mismatch 属实但根因=过河拆桥不验财生官相连，按根因修复**并非必损**（§6.3 已移出永久必损清单，本行留作锚记录） |
...
| 22期例4/5/6 | 从格根被坏三式（批6 复核无误）；⚠️批6 勘误：**例6 书判从官格（论从）非「破从」**——「未从」是例7，旧记与书相反；误记疑已传导 yongshen.py:255 注释（所引锚干支「癸乙丙丙/酉丑子申」疑误记——代码注释留 F11 修，本批仅 KB 记录该传导）；「比肩再多也无用」（qi22） |
...
- 发财运非反局锚群（11 例）：复例四庚申/资本运营酉/包工头壬卯/富发财戊申/经理-2丙戌/经理-4甲辰/富发财数千万壬辰/煤矿-2壬午/老师午/医师卯/煤矿戌——⚠️批4 勘误：旧记「三类型收紧后 9 干净，残留 2=破从（G5 分类）」已过期；现状 10/11 干净：**资本运营酉运**被 T3 伏吟干被克坏泛化误伤（丢「所透之墓为功神」前提，书明文「行酉运亿万巨富」=假阳），另 **zj 丙戌运**假阳（T1 冲无开库豁免，书明文戌运冲开辰库发财 5 亿）——两伤均在 A14 零测试规则面。
...
- **永久必损**：li244（寿元章附述零财）/张克东/qi31。⚠️批6 勘误：ans12-下岗财会**移出**——mismatch 属实但根因=过河拆桥不验财生官相连，按根因修复并非必损（双锚夹击+局部最优记录保留 §5.2 参考）。
...
| 6 | §5.2 22期例6 | 「例6 破从（日主得根）」 | 修正：例6 书判**从官格**（论从），未从=例7；传导 yongshen.py:255 注释（锚干支疑误记）留 F11 | 批6勘误4 |

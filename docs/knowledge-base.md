...
- **当前成绩**（blind_eval rubric v8，**P4 职业军警新面批 2026-09-18**）：

| 维度 | trainset 294 | heldout 215 |
|---|---|---|
| 官命 | 102✅/13❌ = **88.70%** (n=115) | 48✅ = **72.73%** (n=66) |
| 财命 | 63✅/42⚠️/8❌ = **55.75%** (n=113) | 47✅/13⚠️/9❌ = **68.12%** (n=69) |
...
mangpai/
  ...
  objective/  27 模块：bazi_calc/canggan/changsheng/constants/nayin/shensha/shenshu/
    ...
    shishen(十神单一权威,H-fix-4b)/_relation_utils(pair_in,H-fix-4b)
  subjective/ 30 模块：zuogong_confirm(analyze_zuogong+assess_work_level)/zhengfan/
    ...
    yingqi_subj/narrative/schools(保护)/prompts(保护)/utils(_ensure_*公共helper,H-fix-4b)/
  ...
  tests/
    ...
    calib_assertions.py/.yaml  test_*.py(1158 测,含 test_feishu 34/test_d6b_zinv 12/test_llm_channel 27/test_qianyi 11/test_xiangmao 7/test_key_contract 6/test_inject_faults 82/test_hfix5_* 109/test_credential_gate 20/test_doc_numbers 9/test_snapshot_hygiene 2)
...
- 当前 heldout 职业 46.15% < trainset 48.24%、heldout 财命 68.12% > trainset 53.98%——构成不同，只看集内趋势。
...
- **日支穿月令财→封顶小康（T-教授批，2026-09-18）**：`pair_in(日支,月令,LIU_HAI)` 且月令**本气**五行=日主之财（藏干口径否决——会误中马云戌中辛金，书自反例 zhongji:2323「丁酉日主见了戌则是得财之命」支持本气口径）；仅 tier_idx>2 时置 2+追加文本，≤2 no-op（零抖动）；文案避「下浮封顶」字样（不入凶向直杀链）。锚=chuji:2379-2384/3678-3681+5995（教授）、zhongji:2322-2323（邢铭芬）。
...
- **P1 新检测面/消费边（2026-09-18）**：①**支杀化印**（A8，zuogong_detect 新 type，与杀印相生并列不放宽透干门）：杀不透干+印 active+杀支合印支（六合/半合）+杀支非旬空（日/年并参，shouke:5768 反锚）；**仅入 work_actions 不进 work_types**（化用虚高 4 锚结构性免疫），guanming 同作印化官杀消费（chuji:1369-1371/zhongji:3911·3932/shouke:6648）。②**食合官支**（A19，G9 扩展）：非日柱激活自合柱+柱干食伤+支主气官杀 → '合制·食合官支'（chuji:1751 主席）；**时柱主位门**（规则三）+**官支入墓门**（入墓不做功，lixiangxue:6340 普例1 反锚）。③**贼捕制印/制官杀**（A11，zeishen→guanming 新消费边）：`detect_zeibu_dangshi` 党势级贼捕轴（贼虚透+捕党≥6.0 太旺+捕/贼≥3+原神不救）；贼=印→'贼捕制印' 入印类家族（chuji:380 书记/zhongji:3857 朱镕基/gaoji:11171·11380）；贼=官杀→'贼捕制官杀'（G3 同口径门）。zeishen 既有输出与 engine.py 零改动。
- **P2 fp 窄修（2026-09-18，消费侧两条款，objective 零改动）**：①**A12 女命夫宫域分流**（新面）——`classify_guanming_combo`/`analyze_guanming` 增 `gender` 形参（缺省 None 零行为变化），engine.py 调用补 gender：女命日支（夫宫）参与做功的官杀类 combo（伤食制官杀/劫刃制官杀/官杀制比劫/官杀制伤食）归「夫荣」域不计己官（chuji:2206-2209「此造女命不为当官，因夫宫巳火做功…如果是男命就为官了」+yanjiu:5646 同造重出）；印类/财域劫刃制财/藏杀被制/G9/食合官支/贼捕/生用化用不动；真阳锚 cj-2097（印类豁免+印化官杀兜底）/yx-部长（余非日支 combo 不翻），假阳锚 cj-2206。②**A17 旺杀入墓墓不开不作功**（flag 仿 G6 形态）——官杀主气支≥2（多而墓）全入同一在局墓+墓无冲刑未开（消费 muku.analyze_muku 公共 API）+无官杀做功 combo（含藏杀被制）亦无印化官杀 → 不立官命（chuji:1405/1409+第二独立锚 chuji:3161「墓又不开，不作功，穷命、无官」）；豁免=有官杀做功（曾国藩「功在墓杀」/阎百川「旺杀入墓丑统七杀」lixiangxue:7182=墓统杀为所用，系「功在墓杀 vs 墓不开不作功」书内区分本身）；detail 仅决定性时追加。收档 A13/A14/A15/A16/A18（理由=backlog P2 节/预注册 §三）。
...
- military：官杀主气≥2+阳刃+刃支与官杀动作+未成势+财做功未触发+3（驾杀）；corro 每桶封顶+2；mingju_xiong gating；官杀为忌克身贫贱 gating（官杀主气≥3+身弱/从弱+tier 贫/小康 → 撤 military/lawyer）。**F15 落地 8.2 组合**（gaoji:11620-11964）：戌武库做功+3/火金相战+2/金水成势见火+2/申酉丑寅≥3字交织+2/丑戌刑·阳制阴+2（制类须阳为制方）/戌武库刑冲开官杀库+2，贵气门=官杀主气≥2柱且透干+组合封顶+6；④比劫库制印不落地（政委例十 vs 复例四双锚同构）；接入决策=不接 gongmen_wuzhi（F1 弃用，11 P0；**F18 正式弃用落地**：narrative 结论行通道切断、阳制阴口径按 gaoji:11787-11788 修正存档），本模块重写；军警书例探针 1/10→3/10（军官例二/纪检例九归位）。**P4 增库制库通道（贵气门**外**，2026-09-18）**：「库制库·阳制阴（墓用执法象）+6」——要件与门=① `muku.detect_ku_zhi_ku` 命中（阳库辰戌 收/刑 阴库丑未，阳为制方）② 阴库成双多见（丑≥2 或未≥2，墓用结构条件2「有物可墓…成势、多见」gaoji:2190-2194+案例三双丑/例四双未明文；单库孤见不成墓用——复例四/罗斯切尔德/乔布斯/yx-科级结构性不命中）；墓用结构=格局级做功（gaoji:2177-2182）不走贵气门（:11956 管 8.2 字级组合）；mingju_xiong 军警 gating 照旧撤分（li139/qi05/li141 实证）。书锚 gaoji:2401-2417（阳库制阴库=警察）+:11747-11756（戌未刑开杀库=兵营掌权）+:11630（丑=阴库公安象）。探针 3/10→4/10（军官例四归位，F15 贵气门备案由本通道承接）；锚 margin：例二 mil 10（margin 6）/例九 mil 11（margin 1）条款不命中逐字不动。
...
- **F2 已修 muku 三 P0**：①多而墓之计天干（理象学:3002-3005「天干地支合在一起…辛酉柱见丑即入丑墓」，is_entomb 增 all_gans 形参）；②四库之土直接入辰墓去「多」条件（:3008「丑入辰墓，未也入辰墓」无多前提+书例 :3080-3084 未入辰；旧注释「唯多方收」反托段氏已更正）；③TOMB_MAP 戌传导（见上）。哨兵=test_muku.py 8 测（先红后绿）。**P4 增 `detect_ku_zhi_ku`（纯增量，2026-09-18）**：库制库=阳库（辰戌，阳土）收/刑 阴库（丑未，阴土）检测（收式唯辰——戌论冲开不入墓故土支入戌不成立；刑=丑戌/戌未；阳库冲阴库十二支不存在，kind∈{收,刑} 完备）；analyze_muku/is_entomb 既有输出零改动，消费方 13 调用点契约不变（全量回归=六件套实证零回归）；唯一消费方=zhiye military（§4.8）。
...
| cj-2206 | 女命夫宫做功=夫荣非己官（~~A12 fp 窄修残留~~ **P2 已清**：女命日支参与做功官杀类 combo 归夫荣域，chuji:2206-2209+yanjiu:5646） |
| cj-1395 | 「旺杀入墓，墓又不开，所以不当官」（~~A17~~ **P2 已清**：多而墓+未开+无官杀做功不立官，chuji:1405/1409+第二锚 chuji:3161） |
...
| gj-警察墓库 | 「阳库（辰）收阴库（丑）…阳制阴，有执法、纠正之象…实际为警察」（gaoji:2401-2417）=P4 库制库通道主锚（**P4 已 ✅**）；军官例四 gaoji:11747-11756「戌未刑开杀库…入兵营掌权」=同通道第二锚 |
...
### 6.2 官命残留 13❌（P2 后；原 19❌，P1 检测簇清 4、P2 fp 簇清 2）

- ~~fp 窄修簇 A12 女命夫宫/A17 墓不开~~ **P2 已清**（cj-2206/cj-1395 ❌→✅，见 §4.7 P2 条）。**fp 簇余 5 收档**（P2 裁定不机制化）：A13 争合官无力（reg67-申机器工人 zhongji:1233，单例孤锚+与 R3GUAN 合身锚冲突）/A14 合绊无功（zj-教师无官 zhongji:2308，须否决印类 combo 触方向门禁令）/A15 合用官被穿破（cj-老总 chuji:3258，印化官杀独立立官撤 combo 不足翻判；KB 旧标「制不尽」与书原文机制不符，以书为准）/A16 禄上坐官（cj-平辛辛苦苦挣钱 chuji:2285，孤例+须动印类）/A18 制财尽（cj-巨富制尽 chuji:916-922，与 G7 窄豁免锚 cj-县长同构不可分）。保护锚 cj-2097/yx-部长/朱元璋/公安 P2 margin 检验全保。
- ~~检测簇 A8 支杀化印/A11 贼捕入官/A19 食合官支~~ **P1 已清**（cj-正处级化杀/cj-书记/cj-主席 ❌→✅；新检测面×2+新消费边×2，见 §4.7）。
- C 备案 7（原 8，**戴笠 P1 经 A11 贼捕制印路径修复**——机制类书锚 chuji:380/zhongji:3857，该例书文机制=制财军权与 G1 冲突维持不修）：zhenbao-01（与岳飞同构无法区分）/cj-演员单田芳（书内矛盾）/cj-平财不大/cj-1687/cj-包工头（管理≠官）/生例四（体制内口径）/县长-3（运引非原局）。
...
- 残留❌9（heldout）/8（trainset——T-教授批清 2：cj-教授 ❌→✅（日支穿月令财封顶小康条款，终判批可立候选已立项落地）、yx-贫穷命贫困线上 ❌→⚠️ 移出❌清单；旧记 10=D1 数据批 gold 修正后实测，修批C 记 11、旧记 8/22 系批4 收官口径，R4）主簇：~~A13 制库基阶~~（**P3 已清⚠️侧 2 例**=富火运/制例二）/~~A4 伤官见官（土金伤官条款）~~（**P3 收档**=N1 成势怕见官 severe 已承担传导，scored 残余=零，过河拆桥富锚反向锚）/~~G5 破从残/A1 反局残~~（**终判批收档关闭**=方案 A 节：收益噪声带内不可证+方向总线风险+F7/F11 已修尽+无双锚；重启条件=final-state §四）/~~A12 体坏未入凶向链~~（**P3 收档**=N5 已接入 mingju_xiong，独眼乞食 ✅）/~~cj-教授~~（**T-教授批已立清账**：日支穿月令财→封顶小康，双锚 chuji:2379/3678+zhongji:2322，❌→✅）。❌8 逐例处置=P3 预注册 §四+终判批/T 批（zhenbao-14a calib 常驻/cj-种地/yx-破财那几年/yx-建筑/cj-包工头 备案原位；yx-贫家境贫寒一贫 阳日见阴官族 收档候选原位）。**终判批（2026-09-18，docs-only）财 3 例终判**——cj-装璜（⚠️差1）：不净封顶漏检族属实（`_assess_zhi_jing` 无「用神透干」信号，chuji:4755-4766），509 扫改善 4⚠️ vs 误伤 3✅巨富锚（含 cj-巨富制尽书锚正面冲突）+孤锚 → **收档**（重启条件=final-state §一.1）；cj-妓女：官入主位墓不开伤财机制（chuji:2334）可修同构面=1 例+无双锚+与 N3 主位墓豁免语义冲突 → **收档关闭**；cj-教授：日支穿月令财限小康机制**双锚齐**+同构 3 例+✅误伤面 0 → ~~可立窄条款候选~~ **T 批已立项落地**（详账=backlog T-教授节）。详账=`docs/final-state-20260918.md` §一。
...
| `python3 -m pytest mangpai/tests/ -q` | 1159 collected（**1158 passed+1 xfailed，文档数字闸门批实测**；旧记 脱敏闸门批 1149、审查修复批 1129、T-教授 1113、P4 1107、P3 1097、P2 1087、P1 1077、L1 1066、842/822(N2b+N3)、794、776、682、499、473 均作废；19 xp 死测试 H-fix-4a 随 chuangong 删除） |
...
| `scripts/check_doc_numbers.py` | **文档数字闸门**（2026-09-19 入库）：README/KB 权威声明位（首行/验证表/架构表/验证口径行/本表 pytest 行）模块数与 pytest 数字 vs 实测（`pytest --collect-only` ~0.5s + 分层口径计数：foundation/objective 递归、subjective 只算顶层），漂移即退出码 1；`--quiet`/`--json`；verify 四件套=低频项跳过（总数运行期动态累计，静态解析不可靠）；**非 pre-commit 默认项**，改了测试/模块数后手动跑 → 常驻闸门六件套（分层/typing/键契约/防假-green/脱敏/文档数字） |
...
python3 -m pytest mangpai/tests/ -q               # 1159 collected（1158 passed+1 xfailed，文档数字闸门批实测）
python3 mangpai/tests/heldout/blind_eval.py --out snapshots/<批>.json --note "<验证状态>" --baseline latest
...
python3 scripts/check_layering.py && python3 scripts/check_typing_imports.py  # H-fix 新增两件套
```

---

## 9. 当前基线与残留总账（2026-09-18 **T-教授财命窄条款微批落地**：caiming 消费侧单点条款「日支穿月令财→封顶小康」（月令本气口径，双锚 chuji:2379-2384/3678-3681+5995、zhongji:2322-2323；objective/总线零改动）——终判批可立候选 cj-教授立项落地，两道必答题实跑作答（马云本气口径结构性不命中+藏干口径误中对照证明；§7-15 身弱财旺 cap 否决记录无交集）；trainset 财 61→63✅（+2 全预注册：教授 ❌→✅/邢铭芬 ⚠️→✅/贫穷命 ❌→⚠️改善，M3 噪声带内，CI 下界 44.8%→46.6%）、heldout 三维零翻转零抖动（官 48✅/财 47✅/职 24✅ 保）、官 102✅/职 41✅ 零翻转、文本抖动 1 条全归因（yx-书法家-2 条款文本；佛具/普金岁运反局先行压档条款 no-op=良性偏差备案）、富命锚全保（马云/李嘉诚/保尔森/奥纳西斯/煤矿谓词全 False）、67/famous 无变化、calib 常驻 2 条零新增、pytest 1113+1xf、双 seed 逐字节一致、分层两件套通过、3.11.15/3.14.4 双版本 blind 冒烟剥 _meta 逐字节一致（blind 链路不触 sxtwl）；换基线 `snapshots/20260918_t1.json`+LATEST 推进，详账=backlog T-教授节/CHANGELOG T 条/预注册 docs/kimi-t-jiaoshou-caiming-prereg-20260918.md；财命残留 ❌10→8（教授已清、贫穷命移出❌清单）。前一棒=**终判批落地·本周期正式收官（docs-only 零代码）**：财 3 例终判——cj-装璜/cj-妓女 收档（前者漏检族属实但误伤 3✅巨富锚+孤锚风险收益倒挂，后者同构面=1 铁律 4 不达+与 N3 主位墓豁免语义冲突）、cj-教授 可立窄条款候选（双锚 chuji:2375+3674/zhongji:2316+同构 3 例+✅误伤面 0，**本批不实施**另行立预注册微批，必答题=famous-马云 collateral）；G5 破从/A1 反局收档关闭（重启条件=final-state §四）；官 ❌13/职 ❌32 收档确认（全例已判）；feishu 并发两 P1 随群聊决策关闭；A3 维持条件项；上线 checklist #4 群聊关闭、#3 冒烟待凭证（范围=单聊）；**55 例残留全例有终判、零悬置**，终态汇总=`docs/final-state-20260918.md`，落档=backlog 文末终判批节+收工 remaining-tasks-20260918 终判节。基线不动=`snapshots/20260918_p4.json`。前一棒=2026-09-18 **P4 职业军警新面批落地·引擎精度批 P 系列收官批**：objective muku 纯增量新检测 `detect_ku_zhi_ku`（库制库·阳制阴=阳库辰戌收/刑阴库丑未，既有函数零改动）+zhiye military 消费条款 +6（要件=检测命中+阴库成双多见，贵气门外格局级墓用结构通道，军警 gating 照旧）。预注册先行（`docs/kimi-p4-zhiye-muku-prereg-20260918.md`）；哨兵 `test_p4_zhiye_kuzhiku.py` 10 测先红后绿。**trainset 职 40→41✅（+1 全预注册= gj-警察墓库 merchant→military tie_pri 胜出；⚠️ 预警带 +2~4 未达=偏差备案：库制库书锚仅 2 例同构，铁律4 不为追指标放宽）**；**heldout 三维零翻转零抖动（官 48✅/财 47✅/职 24✅ 保）**；trainset 官 102✅/财 61✅ 零翻转；**文本抖动 0**；命中面 12 例全归因（数亿坐牢/qi22 无职 verdict 快照不收、足球/建筑-2/8721/qi50 分升 primary 不变、li139/qi05/li141 gating 撤分——li141=岁运反局流年破坏功神）；军警探针 3/10→**4/10**（军官例四戌未刑开杀库归位，F15 贵气门备案由本通道承接）；例二（margin 6）/例九（margin 1）锚条款结构性不命中逐字不动；yx-科级 collateral 不复现（阴库 0）；muku 消费方全量回归=供给契约零改动声明+六件套实证（13 调用点零回归）；六件套全绿（pytest 1107+1xf、双 seed 逐字节一致、67/famous 无变化、calib 常驻 2 条零新增、分层两件套通过、3.11/3.14 冒烟逐字节一致）；换基线 **`snapshots/20260918_p4.json`**+LATEST 推进；详账=backlog P4 节/CHANGELOG P4 条；收档=8.2 例三/五/六/七/八/十+军警备案簇余+「库藏何物」分级加权。**P 系列收官终态**：P1 官+4/P2 官+2/P3 财+2/P4 职+1 四批全落地，trainset 官 96→102✅/财 59→61✅/职 40→41✅，heldout 三维四批贯穿零回退；方案单列项=财命 G5 破从/A1 反局（高风险单列或收档，方案四节已定）；残留=官 ❌13/财 ❌10/职 ❌32（§6.1/§6.2/§6.3）。前一棒=**P3 财命残簇批落地·引擎精度批 P 系列第三批**：前提复核收档 A4（土金伤官怕见官传导已由 yongshen N1「成势怕见官」severe 条款承担，509 例探针 scored 残余目标=零，b67-过河拆桥富锚为反向锚）/A12（体坏=N5 `detect_zhuwei_ti_chonghuai` 已接入 mingju_xiong，yongshen.py:1682-1684，独眼乞食 ✅ 残余=零）；落地 A13 制库基阶落位两条款（caiming 消费侧，objective 零改动）——条款一上限=制库独力上浮封顶富、库同藏官杀豁免（`_detect_zhiku_decai` 增输出 `ku_han_guansha`；保端 yx-煤矿 yanjiu:7689-7691/奥纳西斯 L4 直达，杀端 cj-富火运 chuji:5526-5530/reg67-制例二 lixiangxue:6478-6484）+条款二下限 sticky=制库在档明财阻断落富不落小康（收束 floor「升后复降」矛盾）。预注册先行（`docs/kimi-p3-caiming-prereg-20260918.md`）；哨兵 `test_p3_caiming_zhiku.py` 10 测先红（5）后绿。**trainset 财 59→61✅（+2，方案带 +2~4 下沿；M3 噪声带内，CI 下界 43.1%→44.8%）**——cj-富火运发财数百/reg67-制例二全预注册；**heldout 三维零翻转（官 48✅/财 47✅/职 24✅ 保）**；trainset 官/职零翻转；文本抖动 7 条全归因（条款一 adjust×5+条款二 li240+入狱一年 tier_static 巨富→富，财维全 unscored 或全量轨终值不变）；M2 七组无失衡恶化（trainset 富组 13✅→15✅ 改善、余六组逐字不动，heldout 七组不动）；富命锚复验全保（李嘉诚/保尔森 zhiku=False 结构性不动、煤矿豁免保巨富、奥纳西斯不动）；凶向标注全量轨红线复查通过；六件套全绿（pytest 1097+1xf、双 seed 逐字节一致、67/famous 无变化、calib 常驻 2 条零新增、分层两件套通过、3.11/3.14 冒烟一致）；换基线 **`snapshots/20260918_p3.json`**+LATEST 推进；详账=backlog P3 节/CHANGELOG P3 条；财命 ❌10 原位（逐例处置=预注册 §四，非三簇者入收档候选；P4=职业军警墓库做功为下一批）。前一棒=**P2 官命 fp 窄修簇批落地·引擎精度批 P 系列第二批**：消费侧两条款全落地，objective 零改动——A12 女命夫宫域分流（`classify_guanming_combo`/`analyze_guanming` 增 gender 形参，engine.py 编排侧接线，缺省 None 零行为变化）+A17 旺杀入墓墓不开不作功 flag（消费 muku.analyze_muku，功在墓杀豁免）。预注册先行（`docs/kimi-p2-guanming-prereg-20260918.md`，fp 簇 7 例定位+双端锚+四保护锚 margin+收档清单）；哨兵 `test_p2_guanming_fp.py` 10 测先红（5）后绿。**trainset 官 100→102✅（+2，方案带 +2~5 下沿；M3 噪声带内，CI 下界 79.6%→81.6%）**——cj-2206（A12）/cj-1395（A17）全预注册；**heldout 三维零翻转（官 48✅/财 47✅/职 24✅ 保）**；trainset 财/职零翻转；文本抖动 2 条（cj-妓女/shouke-qi23 veto_reasons 岁运剥除连锁，A12 设计内，官维 unscorable）；收档 A13/A14/A15/A16/A18 五条（A15 实测机制=合用官被穿破非「制不尽」，以书原文为准）；四保护锚 margin 检验全保+A4 印类方向门 10 锚复验全 True；六件套全绿（pytest 1087+1xf、双 seed 逐字节一致、67/famous 无变化、calib 常驻 2 条零新增、分层两件套通过、3.11/3.14 冒烟一致）；换基线 **`snapshots/20260918_p2.json`**+LATEST 推进；详账=backlog P2 节/CHANGELOG P2 条；残留官命 ❌15→13（§6.2，fp 簇余 5 收档+C 备案 7+散落 1）。前一棒=**P1 官命检测簇批落地·引擎精度批 P 系列首批**：新检测面×2（A8 支杀化印=zuogong_detect 新 type 并列不放宽透干门、仅入 work_actions 不进化用链；A19 食合官支=guanming G9 扩展，时柱主位门+官支入墓门）+新消费边×2（A11 贼捕制印/制官杀=zeishen_bushen 新公开 `detect_zeibu_dangshi` 党势级贼捕轴→guanming combo append 仿 A7 先例；engine.py 零改动）。预注册先行（`docs/kimi-p1-guanming-prereg-20260918.md`，受影响书例清单+双端锚+4锚/10锚）；哨兵 `test_p1_guanming_zeibu.py` 11 测先红后绿。**trainset 官 96→100✅（+4，方案带 +3~5 内；M3 噪声带内，CI 下界 75.6%→79.6%）**——cj-正处级化杀/cj-书记/cj-主席/cj-戴笠（机制类书锚路径已声明）全预注册；**heldout 三维零翻转零抖动（官 48✅/财 47✅/职 24✅ 保）**；trainset 财/职零翻转；文本抖动 1 条（zj-平常八字 veto 剥除，A8 门槛设计内）；化用虚高 4 锚+A4 印类方向门 10 锚复验全保；六件套全绿（pytest 1077+1xf、双 seed 逐字节一致、67/famous 无变化、calib 常驻 2 条零新增、分层两件套通过、3.11/3.14 冒烟一致）；换基线 **`snapshots/20260918_p1.json`**+LATEST 推进；详账=backlog P1 节/CHANGELOG P1 条；残留官命 ❌19→15（§6.2，P2=A12+A13-A18 可机制化者为下一批）。前一棒=**L1 遗留清理批落地·引擎精度批紧前批**：阶段甲零输出 5 项（B2 liunian 入口守卫/B3 `_auto_liunian_injected` 初始化/B4 冗余删除/C2 DISCLAIMER 单源/C3 快照卫生测试）blind vs hfix7 零翻转零抖动；阶段乙（B1 xiangfa_ops 排序化 4 处+同族补漏 frozenset join 2 处、C1 zaihuo 官杀 label 计数不动）抖动归因白名单外 0 路径后一次换基线 **`snapshots/20260918_l1.json`**+LATEST 推进；**双 seed 复现达成**=509 例 payload 特征 JSON seed 0/7/42 全一致（修前 287 例序差清零，残留 68 例全限 relations 内部总线键=B6 关闭域）；六件套全绿（pytest 1066+1xf、calib 常驻 2 条零新增、分层两件套通过、3.11+3.14 冒烟 ok）；详账=backlog L1 节+CHANGELOG L1 条。早前=2026-08-22 **修批 G2 样式批落地 → G1/G2 收官·系统发布态**：F6-7/F6-8/模板语三令+统计口径清零+F6-3/4/5 结构性残留清零+注释/mock 归并，六件套全绿，基线推进=`snapshots/20260822_g2.json`；上线 checklist 6/8 闭环（真实凭证冒烟/群聊 @bot 后置）；收工 `docs/remaining-tasks-20260822.md`。前一态=**修批 G1 落地 → 七维正式发布 GO**：F6-1/F6-2/复合词真漏网三阻塞项全清+F6-6 顺手并入，六件套全绿，基线推进=`snapshots/20260822_g1.json`；统计口径项裁定后置 G2。前一态=第六轮审查 W1-W5 收官（只审不改零 API，引擎/生产代码零改动，基线=`snapshots/20260821_n3.json`）：六轮全量=P0×0、P1×4（F6-1 降级丢两维/F6-2 误报窗逗号漏杀+死词近义词缺口/复合词真漏网 标致水灵清秀端庄/统计口径——前三项阻塞、统计口径裁定可后置）、P2 若干（F6-3~F6-8 等，全不阻塞）；**七维正式发布 NO-GO**，修批 G1（发布闸四项，一次六件套验收）落地后转 GO；统一待修清单=`docs/tasks/review6-fix-backlog.md`（review5 清单 F1/F2/F3 全落地收官）；发现摘要见 §6.8。前一棒 2026-08-21 动工批 N2b+N3 · 相貌维小修续打+七维收档收官（用户选 a 续打，引擎零改动）：r3 残留「优美/柔美」复合词族以书锚为准**入禁**（书内相貌结论词仅 漂亮/美貌/秀气/好看/曲线好，复合评价词零锚；校验器单字「美」扫描已全覆盖码不动，llm_prompt SCHEMA 相貌条款+`_xiangmao_anchor` 禁令显式点名「曲线优美/体态柔美/秀美/俊美」同禁；哨兵 test_xiangmao_redline_n2b_compound 先红后绿）。r4 复测 294 例谷段：**相貌维红线 2→0 加严线达成、迁移维红线 0 保**；L0=0、L1=1.36%（4 例杂键旧族压 E7 线）、N1=1 例（reg67-合例六两妻「一次婚」计数=单例采样抖动，r1 同族）、L2 既有=1 例 0.34% 压线（gj-低保伤官「守成则平」告诫式疑似假阳族 mark 留人工，同 E7/N2 处置）；成本 ¥21.08 预算内。N3：blind vs gap2 heldout/trainset 零翻转零抖动（官 48✅/财 47✅/职 24✅ 保）+双 seed 一致+verify×4 全绿+67/famous 无变化+calib 常驻 2 条零新增+pytest 822+1xf+19xp，基线=`snapshots/20260821_n3.json`。**七维叙述终态指标：迁移维红线 0/294、相貌维红线 0/294、新维翻转 0/30（S1 裁决后）、既有五维 L0 0 / L1 1.36% / N1≈0 / L2 0.34% 全压 E7 线内**；发布 **GO**（F1+F2 落地+六件套复跑全绿）+七维 **GO**（新维加严线达成）→ 统一动工方案 F+N 全链收官。收档=CHANGELOG N1/N2/N2b 条目+收工 remaining-tasks-20260821+方案归档标完成；报告 docs/kimi-n2b-n3-final-report-20260821.md。前一棒 动工批 N2 · 七维复测（三轮收敛：L2 相貌 38→3→2，迁移维全绿 红线 0/翻转 0/锚定引用 264/264；S1 评审 30 例裁决后新维翻转 0/既有五维翻转 0/放大 6.3%；judge 一致率 91.2% 召回 0/2 降级筛子；F-V3-1 zhenbao-23a 失业直述修复确认；成本 ¥67.2≤$12；F-N2-1 引擎侧 xiangmao.py:111「漂亮」冻结留后续引擎批；报告 docs/kimi-n2-retest-report-20260821.md）。前一棒 动工批 N1 · 七维叙述代码批：DIMENSIONS 五→七（+迁移+相貌）+L2 两按维红线（迁移绝对禁出境词/相貌禁美丑结论词带美元/丑时/X丑排除窗）+`_qianyi_anchor`/`_xiangmao_anchor` 锚定行+哨兵先红后绿；F1+F3 代码 11 项随 f3d3e5e 并行落地（免责语/Tuple/lark_md/死亡词 mark→reject/外发告知/丁眼锚注/isinstance/max_tokens 8192 等）；294 例探针全通；pytest 821+1xf+19xp；引擎零触快照链不动。前一棒 2026-08-21 修批 F2 · 文档清零（纯注释+文档零行为）：第五轮审查 V1-V6 入档（§6.7 发现摘要+CHANGELOG+收工 go/no-go=NO-GO 待 F1+F2 落地转 GO）；锚注清零=qianyi zhongji:4179→4180（5 处+测试注/函数名）、「结构同构」措辞改准（一合一冲不同构、立论成立）、subjective/__init__ docstring 40→41；待修清单标认领（F1 #1-6/F3 #11-15）。pytest 复跑 794+1xf+19xp 无意外；引擎零改动，基线仍=`snapshots/20260820_gap2.json`。前一棒 2026-08-20 缺口批3 · 三项收档+KB/收工全量同步（纯文档零代码）：世应/风水化解/时空测事收档口径入 §6.6；KB 补同步批1/批2——§1.1 模块 29→31、§4.14 qianyi/§4.15 xiangmao 新条、selectors 41、pytest 794+1xf+19xp、快照链 e3→gap1→gap2；引擎零改动，基线仍=`snapshots/20260820_gap2.json`。前一棒 缺口批2 · xiangmao 相貌 marker 层：4 主线（秀气透干 zhongji:3914+lixiangxue:6655 反条件/金水伤官限辛 zhongji:1484+shouke:5394/活木见火 zhongji:4513+chuji:4371+lixiangxue:6628/眼象丙丁癸 zhongji:1482-1483）+2 弱线（伤官合官杀魅力 gaoji:5618-5623+shouke:634-638/身材曲线 zhongji:3981+1484），纯 marker 无判定，selectors 40→41，哨兵 test_xiangmao 7 测先红后绿，pytest 794+1xf+19xp，blind vs gap1 零翻转零抖动，基线=`snapshots/20260820_gap2.json`。前一棒 缺口批1 · qianyi 迁移/远行：原局三 marker（月日冲 gaoji:5857/日时合 gaoji:5858/马临年时 gaoji:6735）+应期窗（马逢冲 shouke:3602/合到门户 zhongji:4179+lixiangxue:6571 双锚/伏吟冲出或然），措辞上限「迁移/远行」不出「出国」，selectors 39→40，哨兵 test_qianyi 11 测先红后绿，pytest 787+1xf+19xp，blind vs e3 零翻转零抖动，基线=`snapshots/20260820_gap1.json`。前一棒 2026-08-19 修批 E7 · 迭代 7+文档同步：官命矛盾 4 例全误判（否定出窗）→官命维窗 ±2→±5；prompt 锚定补「可达/可至」；校验器⑦补「虽」让步窗+「档就是/档为」归位标记治 E7 新发 3 假阳；294 例谷段复测 L2 财档 2→1（0.34%，残留=单例真越限抖动 mark 留人工）、官命 4→0；pytest 776+1xf+19xp 全绿；引擎零改动，基线仍=`snapshots/20260819_e3.json`。前一棒 修批 E6 · 财档迭代 6：校验器口径修 6 条+prompt 锚定，L2 财档 16→2（0.68%），$2.98 谷段。前一棒 修批 E5 · 飞书加固（引擎零触）：重放窗口滚动/token 锁/静默错解三例/500 脱敏/HTTPServer 上限，test_feishu 28→34，pytest 773。前一棒 修批 E4 · 引擎裁定批：穿引动裁定 a·改注（纯注释零行为，口径备案见 §4.13）+损子「冲」收档（孤锚不立）；pytest 767+1xf+19xp 全绿，blind vs e3 零翻转零抖动，基线仍=`snapshots/20260819_e3.json`。前一棒 修批 E2 · 文档清零批：KB/CHANGELOG/收工文档按 U4 漂移清单清零，引擎零触。前一棒 修批 E1 · 飞书上线必修（纯 feishu 包内引擎零触）：client.py 捕获 token 99991663/99991661 清缓存重取重试一次（正常路径零影响）+bot.py reply 包 try 纯文本兜底重发一次+EncryptKey 检测告警丢弃+README 红线「勿配 Encrypt Key」+main() 启动 VT 强制（FEISHU_VERIFICATION_TOKEN 未配 RuntimeError+上线 checklist）；哨兵 test_feishu +5→28 全绿，mock 冒烟四条全过，U4 飞书条件 GO 三 P1 清零。前一棒 飞书集成工程批（bb4decf）：mangpai/feishu 新包 6 文件 528 行（client 带 TenantAccessToken 缓存/router/service/formatter/bot+README）+test_feishu 23 测，mock 全链三例跑通，U2 审查 P0=0（token 刷新/重放/降级三判据全过，fuzz 232 例零崩溃），引擎零触。前一棒 修批 D6b · 子女断法实现：zinv 新模块立 4 项（得子 3 机制/损子 5 机制/借腹 marker/时柱喜用腿落 liuqin，详见 §4.13），selectors 38→39（verify_dayun:405 断言同步），哨兵 test_d6b_zinv.py 12 测先红后绿，pytest 739+1xf+19xp，blind vs d3 零翻转零抖动，基线=`snapshots/20260819_d6b.json`；D6a=纯设计批零实现。前一棒 修批 D5 · 工具/备案批（收尾，引擎零改动）：`_llm_batch_rescore.py` sorted glob 合并确认 D4 已修+冒烟验证 retry 覆盖生效（原记录无 reading 被 retry 覆盖后重评分例数=1）；G6 scrub/as_of_year/子夜带三备案落 §4.11；pytest 727+1xf+19xp 全绿。前一棒 修批 D4 · prompt 迭代 5：职业桶/应期逐年锚定入 llm_prompt，引擎零改动；S1 复验翻转 9/30→0/30 三线全达标，飞书集成 GO，详见 docs/kimi-d4-prompt-iter5-20260819.md。pytest 727+1xf+19xp。前一棒 D3 供给批更新：dayun_analysis 死 selector 修复——选 B 补供，build_payload 层合成大运表（dayun_gz_sequence 方向+8 步干支序列 + analyze_dayun_mangpai 信号），engine compute_all 零改动；合成路径起运岁诚实缺省（缺精确出生时刻），每运投影 14 字段与 prompts/mangpai.md 引用对齐，D4 应期锚定直接可用；payload +约 7180 字符≈4500 token/命；顺手修 verify_dayun selectors 断言 39→38（70/70）。前一棒 D2 入口批：性别必填+界外年份 guard+lon 校验，引擎判定零改动；再前 D1 数据批 gold 修正 5 条+source 锚 15 处+raw_quote 1 处+calib zhenbao-10 dayun 误录删除）
...
- **H-fix 系列（2026-09-17~18，代码卫生修批，每批引擎判定零改动+blind 零翻转零抖动）**：H-fix-1 import 崩溃面 → 2a/2b/2c 异常策略三层（吞改抛/传导/降级+注入框架 test_inject_faults 79 测）→ H-fix-3 原子写 19 处+llm_channel 免责提级 → H-fix-4a 死代码清理（advanced/chuangong 删模块）→ **H-fix-4b 重复逻辑下沉（_check_pair/十神/_ensure_*/相貌判定/detect_relations 6 循环注册表）** → **H-fix-4c 局部 import/循环依赖整理（生产局部 import 93→8、yongshen 星型回边全上提、gongliang⇄caiming 方向矫正顶层图无环、分层检查 scripts/check_layering.py 入库）** → **H-fix-6 selectors/engine-keys 契约测试（test_key_contract.py 6 测三方集合断言：engine 48=selectors 41+内部白名单 7，payload=selectors 锁 41，特征直喂备案 chang_sheng/narrative、zinv 显式标注预留；红验证删/加键皆红；阶段 2 裁定不引入自动派生）**；快照链 g3→**20260917_prehfix→20260917_hfix1**→hfix2a→20260918_hfix2b→hfix2c→hfix3→hfix4a→hfix4b→hfix4c→hfix6→hfix5→hfix7→**20260918_l1**→20260918_p1→20260918_p2→**20260918_p3**→20260918_p4→**20260918_t1（当前基线，`snapshots/LATEST` 指针所指向）**；pytest 1113 passed+1xf（P1 +11 测、P2 +10 测、P3 +10 测、P4 +10 测、T +6 测）；各批差异分析/六件套登记=`docs/tasks/codehygiene-fix-backlog.md` H-fix 节；新公共模块 objective/shishen.py·objective/_relation_utils.py·subjective/utils.py·output/_eval_common.py（分层单向不破，后者=评测工具脚本公共层不入分层图）。**H-fix-7 评测框架统一（🟢 可选批，2026-09-18）**：output/ 四组重复（runner/校准/抽样/检查）下沉 `_eval_common.py` 五节，11 脚本薄包装（接口/schema 不变），口径对拍全绿（prompt 575 例/校准 4 批/抽样 2 批/w4 294 例/n2a/t3d 固定 seed 逐字节一致），顺带修 H10 P0 `_w5_crosscheck` ImportError（_xm_sanitize 直传化）；随机 seed 序差=xiangfa_ops 存量备案。**H-fix-5 大函数拆分（H-fix 序列最后一批功能批，逐字等价）**：detect_relations 772→~150 行编排器+13 子函数、analyze_gongliang 949→154 行+5 阶段子函数（H11 方案）、compute_all 489→38 行+5 阶段子函数（①拆 base/extended 夹②，偏差已备案）；哨兵 test_hfix5_* 三文件 +108 测（先红后绿）；等价性=809/1627/518 样本 canonical sha256 对拍逐字节一致（hashseed 0/7/42 三重）；契约测试 6/6 锁 48=41+7；新发现存量问题入 backlog（xiangfa_ops set 迭代序随 PYTHONHASHSEED 旋转等 6 条，不修）。**H-fix-8 文档/基线同步批（🟢 收尾，2026-09-18，H-fix 序列最后一批）**：README 数字同步（59 模块/1059+1xf/trainset 294）、heldout README 23→294+MANUAL 4 例旁路备案（H9 P1 结案）、`snapshots/LATEST` 指针+`--baseline latest`+14 份无引用快照归档 `snapshots/archive/`+`snapshots/README.md` 快照链总账、output sys.path.insert 保留备案（H10 结案：非包+安装化成本高，批跑工具非生产代码）、CHANGELOG 补缺 7 条、收工 `docs/remaining-tasks-20260917.md`；六件套全绿+check_layering/check_typing_imports 通过。**序列终态**：裸 except 残留 3 处（feishu/bot.py×2 边界兜底合规+verify_heldout.py:51 失败通道合规）、模块 58→59、新工具 check_layering/check_typing_imports、契约测试 6 测、原子写 19 处、评测框架统一；明示暂不修=feishu 并发两 P1（群聊上线前）/magic numbers ~45 处 P2（v2 计划⏸️节）。**L0 遗留关闭标记批（2026-09-18，docs-only 零代码）**：6 项遗留待议裁定**已关闭**——A1 SHIPAI 两表（修批C 留档决议在先属重复立项）/A2 gongmen_wuzhi（修批A③/F18 锁定+哨兵+契约白名单三层防护）/A4 jiaoyun `if not span`（公开 API 边缘语义非死码）/B5 gongliang 吞异常+13 书例（H-fix-2b 已实质处置+F6 备案）/B6 detect_relations 含 set（内部总线键无泄漏面）/C4 magic numbers（v2 ⏸️ 维持）——落档 `docs/remaining-tasks-20260917.md` §二 + backlog 文末「L0 遗留关闭标记」节，勿再立项。已办=**L1 遗留清理批（2026-09-18 落地**，7 项全落地+同族补漏 frozenset join 2 处，一次换基线 `20260918_l1.json`，详账=backlog L1 节）+ 待办=D 真实凭证冒烟（上线 checklist #3 执行项，随首次上线窗口）；A3 输出面死字段降格条件项（随输出面批顺手）。行号更正：`_prepare_inputs` 死块在 `subjective/gongliang.py:431-433`（非 engine.py）；B2 漂至 `engine.py:577-578`；C1 漂至 `zaihuo.py:319-322`。
...
- trainset 294：官 **88.70%**（13❌=§6.2，P2 后）/ 财 **55.75%**（❌8=§6.3，T-教授后）/ 职 **48.24%**（32❌=§6.1，P4 后）。
- heldout 215：官 72.73% / 财 68.12% / 职 46.15%（P1-P4 四批全量 diff 零回退）。
...
- 三维攻坚正式收官；**P 系列四批（P1-P4）已收官**（2026-09-18）；后续方向（若重启）：财命 G5 破从/A1 反局残簇（高风险单列专项或收档，方案四节已定）、职业中医/军警残留/lawyer 盲区（军警墓库做功=P4 已落地，探针 4/10；余均须新突破面，旧窄通道已尽）。

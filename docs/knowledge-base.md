...
- **当前成绩**（blind_eval rubric v8，**P1 官命检测簇批 2026-09-18**）：

| 维度 | trainset 294 | heldout 215 |
|---|---|---|
| 官命 | 100✅/15❌ = **86.96%** (n=115) | 48✅ = **72.73%** (n=66) |
| 财命 | 59✅/44⚠️/10❌ = **52.21%** (n=113) | 47✅/13⚠️/9❌ = **68.12%** (n=69) |
| 职业 | 40✅/12⚠️/33❌ = **47.06%** (n=85) | 24✅ = **46.15%** (n=52) |

（P1 前：trainset 官 96✅/19❌ 83.48%。P1 +4 全预注册，M3 噪声带内、CI 下界 75.6%→79.6%）
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 1078 collected（**1077 passed+1 xfailed，P1 实测**；L1 1066+1xf、旧记 G2 838+1xf+19xp、批10 499 均作废）+ blind_eval 快照零翻转 + 双 seed 逐字节一致 + `scripts/check_layering.py`（分层单向）+ `scripts/check_typing_imports.py`（typing/import 面）（旧 853 口径 2026-07-17 起作废）。
...
### 6.2 官命残留 15❌（P1 后；原 19❌，检测簇 A8/A11/A19 已清 3 例+戴笠 1 例）

- fp 窄修簇 A12 女命夫宫/A13 争合官无力/A14 合绊无功/A15 制不尽/A16 禄上坐官/A17 墓不开/A18 制财尽（各1例书明文强锚，窄修须保 cj-2097/yx-部长/朱元璋/公安锚）——**P2 批对象**。
...
| `python3 -m pytest mangpai/tests/ -q` | 1078 collected（**1077 passed+1 xfailed，P1 实测**；旧记 L1 1066、842/822(N2b+N3)、794、776、682、499、473 均作废；19 xp 死测试 H-fix-4a 随 chuangong 删除） |
...
python3 -m pytest mangpai/tests/ -q               # 1067 collected（1066 passed+1 xfailed）
...
- trainset 294：官 **86.96%**（15❌=§6.2，P1 后）/ 财 52.21%（❌10=§6.3）/ 职 47.06%（33❌=§6.1）。
- heldout 215：官 72.73% / 财 68.12% / 职 46.15%（P1 全量 diff 零翻转零抖动）。

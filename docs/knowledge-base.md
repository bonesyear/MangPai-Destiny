...
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 787 collected（767 passed+1 xfailed+19 xpassed，修批E1 后实测；U4 记 762 passed（D6b+12/feishu+23）、D5 记 747 collected/727 passed、D4 记 727 同口径；修批B/C 记 682、修批A 记 677、F19 记 668/648、F18 记 664、F17 记 646、F15 记 606、F13 记 585、F12 记 576、F11 记 543、F10 记 536、F9 记 532、F7 记 517、F6 记 529、批10 旧记 499 均作废）+ blind_eval 快照零翻转 + 双 seed 逐字节一致（旧 853 口径 2026-07-17 起作废）。
...
mangpai/
  ...
  subjective/ 29 模块：zuogong_confirm(analyze_zuogong+assess_work_level)/zhengfan/
  ...
  tests/
    ...
    calib_assertions.py/.yaml  test_*.py(767 测,含 test_feishu 28/test_d6b_zinv 12)
...
- **损子窗五机制**：克到位（运干克星到位且不合星，gaoji:14108-14128）/合去（克运中流年合走子息星，gaoji:14108-14128 戊合癸）/穿引动（岁运支穿子息星所临支，gaoji:14295-14312+17465-17484 降权）/枭夺食运（shouke:428）/合神被克（shouke:18-20 戊克壬）。**措辞中性**（子息星受创/子女宫引动，字面无死/夭/丧），LLM 侧由既有 `_scrub_death` 兜底（zinv 不自建过滤）。
...
| `python3 -m pytest mangpai/tests/ -q` | 787 collected（767 passed+1 xfailed+19 xpassed，修批E1 后实测；U4 记 762 passed、D5 记 747/727、修批B/C 记 682、F19 记 668/648、批10 记 499、旧记 473 均作废） |
...
python3 -m pytest mangpai/tests/ -q               # 787 collected（767 passed+1 xfailed+19 xpassed）
...
| 42 | （外部归档） | selectors「35 键」 | 记录：实际 39 键；compute_all 输出 49 键（KB 未载旧值，备查）。**修批A③（2026-08-18）起 38 键——gongmen_wuzhi 摘除（is_wuzhi 98.8% 恒真零信息量，payload 通道切断），engine result 键保留**；**D6b（2026-08-19）起 39 键——zinv 追加（镜像 liuqin 通道进特征 JSON 纯数据），verify_dayun:405 断言=39 实测过** | 批10勘误2 |

...
| 财命 | 61✅/42⚠️/10❌ = **53.98%** (n=113) | 47✅/13⚠️/9❌ = **68.12%** (n=69) |
| 职业 | 41✅/12⚠️/32❌ = **48.24%** (n=85) | 24✅ = **46.15%** (n=52) |

（P4 职 +1 全预注册（gj-警察墓库），M3 噪声带内。P3 财 +2 全预注册，M3 噪声带内、CI 下界 43.1%→44.8%）
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 1108 collected（**1107 passed+1 xfailed，P4 实测**；P3 1097+1xf、P2 1087+1xf、P1 1077+1xf、L1 1066+1xf、旧记 G2 838+1xf+19xp、批10 499 均作废）+ blind_eval 快照零翻转 + 双 seed 逐字节一致 + `scripts/check_layering.py`（分层单向）+ `scripts/check_typing_imports.py`（typing/import 面）（旧 853 口径 2026-07-17 起作废）。
...
- 残留❌9（heldout）/10（trainset——D1 数据批 gold 修正后实测；修批C 记 11，旧记 8/22 系批4 收官口径，R4）主簇：~~A13 制库基阶~~（**P3 已清⚠️侧 2 例**=富火运/制例二）/~~A4 伤官见官（土金伤官条款）~~（**P3 收档**=N1 成势怕见官 severe 已承担传导，scored 残余=零，过河拆桥富锚反向锚）/~~G5 破从残/A1 反局残~~（**终判批收档关闭**=方案 A 节：收益噪声带内不可证+方向总线风险+F7/F11 已修尽+无双锚；重启条件=final-state §四）/~~A12 体坏未入凶向链~~（**P3 收档**=N5 已接入 mingju_xiong，独眼乞食 ✅）。❌10 逐例处置=P3 预注册 §四（zhenbao-14a calib 常驻/cj-种地/yx-破财那几年/yx-建筑/cj-包工头 备案原位；yx-贫家境贫寒一贫 阳日见阴官族/yx-贫穷命贫困线上 身弱财旺+财不做功族 入 backlog P3 收档候选）。**终判批（2026-09-18，docs-only）财 3 例终判**——cj-装璜（⚠️差1）：不净封顶漏检族属实（`_assess_zhi_jing` 无「用神透干」信号，chuji:4755-4766），509 扫改善 4⚠️ vs 误伤 3✅巨富锚（含 cj-巨富制尽书锚正面冲突）+孤锚 → **收档**（重启条件=final-state §一.1）；cj-妓女：官入主位墓不开伤财机制（chuji:2334）可修同构面=1 例+无双锚+与 N3 主位墓豁免语义冲突 → **收档关闭**；cj-教授：日支穿月令财限小康机制**双锚齐**（chuji:2375+3674/zhongji:2316 邢铭芬）+同构 3 例+✅误伤面 0 → **可立窄条款候选，另行立预注册微批**（必答题=famous-马云 collateral 豁免+§7-15 口径边界；本批未实施）。详账=`docs/final-state-20260918.md` §一。
...
| `python3 -m pytest mangpai/tests/ -q` | 1108 collected（**1107 passed+1 xfailed，P4 实测**；旧记 P3 1097、P2 1087、P1 1077、L1 1066、842/822(N2b+N3)、794、776、682、499、473 均作废；19 xp 死测试 H-fix-4a 随 chuangong 删除） |
...
python3 -m pytest mangpai/tests/ -q               # 1098 collected（1097 passed+1 xfailed，P3 实测）
...
- trainset 294：官 **88.70%**（13❌=§6.2，P2 后）/ 财 **53.98%**（❌10=§6.3，P3 后）/ 职 **48.24%**（32❌=§6.1，P4 后）。

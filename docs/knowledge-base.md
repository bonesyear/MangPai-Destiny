...
- **验证口径**：`verify_mangpai.py` 432 项 + `pytest mangpai/tests/` 529 collected（509 passed+1 xfailed+19 xpassed，F6 实测；批10 旧记 499、更旧 473 均作废——F2/F3/F4/F6 哨兵各 +11/+8/+6/+2）+ blind_eval 快照零翻转 + 双 seed 逐字节一致（旧 853 口径 2026-07-17 起作废）。
...
- 阎锡山 L3（书）vs L4（郝金阳）双标准冲突——**F6 已裁：以书（理象学 7188「三层强一点」）为准取 L3**，checkpoint/calib gold 同步改 3；guanming grade_map L3→中高（处级） 与 _RANK_GRADE L3→厅级-省部级口径差留 F12。

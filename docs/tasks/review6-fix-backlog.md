...
| 15 | W3-P2#1 眼象线「大眼」泛化：补锚注（引擎批） | subjective/xiangmao.py:152 | ✅ G3 落地：回书有明锚 zhongji:4531「丙主眼睛大」，补 inline 锚注（1483/4531+lixiangxue:12632） |
...
- F-N2-1（引擎侧冻结项）：`xiangmao.py` 秀气线 desc「漂亮」改「女看秀气倾向」；`_xm_sanitize` 函数连调用删除（llm_prompt/formatter 两路直传）——**✅ G3 已落地（2026-08-22）**。
- F-N2-2 judge 新维翻转召回弱（0/2）：`_n2_eval.py` 判据补眼象线命中示例（含独癸 desc 空不算命中）+ F6-6 程度词/评价词/引申句入 lv=1 + 相貌 red 补复合词——**✅ G3 已落地（2026-08-22）**。
- #15 大眼锚注：回书裁决=有锚（zhongji:4531「丙主眼睛大」明锚+1478 丙食造「女命眼睛大」+lixiangxue:12632 黛安娜大眼睛），`xiangmao.py` 眼象线补 inline 锚注——**✅ G3 已落地（2026-08-22）**。
...
G1（P1 四项）→ 六件套+294 离线重扫 → **七维正式发布 GO** ✅；G2 样式批（#4-#16 归并，哨兵 8 测先红后绿+六件套全绿）✅ 2026-08-22 落地收官；G3 引擎侧收尾（#15 大眼锚注/F-N2-1 秀气线+删 `_xm_sanitize`/F-N2-2 judge 判据）✅ 2026-08-22 落地——review6 清单全清，剩备案观察项。

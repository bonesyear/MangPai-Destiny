# 第六轮审查统一待修清单 + 修批 G 排期（2026-08-22，W5 收官批立；G1 当日落地转 GO）
...
> 发布判定：**G1 发布闸（P1×3 阻塞项）已落地，六件套全绿 → 七维正式发布 GO**（2026-08-22）；P2 余项随 G2。
...
| 4 | **P1** | 相貌引用率统计口径：`_has_xiangmao_marker` 只看 hit 不看 desc，独癸 16 例误计「有 marker」（W2 §3；对齐后 251/251=100%） | output/_n2_analyze.py:53-58 | 改「hit 且 desc 非空」（或补 eye_full 同口径） | ⏸ 后置 G2（W5 裁定可后置：评测脚本 bug 非发布物，实际引用率 100% 已离线证实） |
...
| # | 项 | 位置 |
|---|----|------|
| 5 | F6-6 扩展族（W4 立+W5 全量坐实 ~60+ 例次）：prompt 相貌锚定补「只写象描述本身——禁程度词（明显/很/强）、禁评价词（有神采/明亮/灵动/灵秀）、禁引申气质总结句（艺术气息/灵动之感）」**✅ G1 已并入落地；G2 剩 r5 复跑确认收敛** | llm_prompt.py `_xiangmao_anchor` |

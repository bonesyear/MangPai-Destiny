...
## 终判批（2026-09-18，docs-only 批——零代码改动 · 本周期正式收官）

> 依据：方案 `~/.claude/projects/-root-metaphysics/memory/kimi-remaining-plan-20260918.md`。诊断=diag_case+509 例同构面全扫（只读，临时脚本在 /tmp）。终态汇总=`docs/final-state-20260918.md`。

### 财 3 例终判（55 例残留最后的未终判面）

- **cj-装璜 → 收档**。不净封顶未触发根因=`gongliang._assess_zhi_jing` 不净信号族不覆盖「用神（财）本身透干未被制」（本例壬癸两财透干判净→L4 不封顶；书锚 chuji:4755-4766「壬癸水都透出来了…原局金水是制不干净的」）。509 扫：漏检族属实（窄口径 39 例命中/可评分 12 例），但改善面 4⚠️（装璜/yx-医师/yx-富富有百万/shouke-qi05）vs 误伤面 3✅巨富锚（cj-巨富制尽=chuji:916-922 书锚正面冲突/shouke-li263/shouke-qi14）+2 例更坏——风险收益倒挂+新信号单例孤锚（铁律 4 不达）+须动功量层总线 → 收档。重启条件=第二书锚+3 巨富锚结构性豁免要件（final-state §一.1）。
- **cj-妓女 → 收档（关闭）**。书机制=官带财入主位墓不开损夫伤财（chuji:2334；书内对照例 壬戌/辛亥/己未/丙寅 不在 509 例内）。509 扫同构面 24 例、可修面=1 例（铁律 4 不达）、无双锚、与 yongshen N3「主位墓=制忌自消」豁免（yongshen.py:1224-1233）语义正面冲突、误伤 3✅锚（cj-巨富制尽/gj-公门转商/reg67-资本运营）→ 关闭写明理由，勿再立项。
- **cj-教授 → 可立窄条款候选（本批只诊断不实施，另行立预注册微批）**。机制=日支穿月令财→与财缘薄限小康；**双锚齐**（chuji:2375+3674 / zhongji:2316-2320 邢铭芬「穿了财，故发不了大财，只能发点小财」+丁酉自合反例）+同构 3 例（教授/邢铭芬/yx-贫穷命贫困线上）+✅误伤面 0——铁律 4 达成。预注册必答题：①famous-马云 同构 collateral（封顶小康则 ⚠️→❌ 触 famous 零回归红线，须豁免要件或档位取舍）；②与 §7-15（贫穷命贫困线上身弱财旺 cap 双锚否决记录）口径边界。现有财众攻 cap 不生效原因=须 ≥2 方冲破穿且不接官统财路径（caiming.py:1522/1617）。

### 关闭项落档（终判批裁定，勿再立项）

| 项 | 裁定 |
|---|---|
| G5 破从 / A1 反局 | **收档关闭**（理由=方案 A 节四点：收益 ~1-2 例 M3 噪声带内不可证/方向总线 classify_strength·zhengfan 全链传导风险违红线 7/F7·F11 已修尽书锚充分面/残留无双锚同构铁律 4 不达；重启触发条件=final-state §四） |
| 官 ❌13 / 职 ❌32 | **收档确认**（13/13、32/32 全部已判无遗漏；逐例理由=本文件 P2 节收档清单/P4 节收档清单+KB §6.2/§6.1） |
| feishu 并发两 P1（后台线程无界/`_seen_mids` 非原子） | **关闭**（唯一触发条件=群聊上线，用户已决策群聊 bot 不做；重启群聊时随批激活） |
| A3 输出面死字段 | **维持条件项**（不关闭不立项；随未来输出面/payload 精简批顺手，方案 §C2） |
| 上线 checklist #4 群聊 @bot | **关闭**（用户决策）；#3 真实凭证冒烟=待凭证保留（范围=单聊，`docs/ops-plan-20260822.md` 已同步） |

---

## T-教授 财命窄条款微批（2026-09-18，判定改动批——caiming 消费侧单点，objective/总线零改动）

> 依据：`docs/final-state-20260918.md` §一.3 终判（cj-教授=可立窄条款候选，两道必答题）；预注册=`docs/kimi-t-jiaoshou-caiming-prereg-20260918.md`；基线 p4→**t1**。

### 条款与实现

| 项 | 内容 |
|---|---|
| 条款 | **日支穿月令财 → 封顶小康**（`tier_idx = min(tier_idx, 2)` 语义，实现为 `tier_idx>2` 时置 2） |
| 谓词（窄，不泛化） | `pair_in(日支, 月令, LIU_HAI)`（穿对表=objective/constants.py:79 子未/丑午/寅巳/卯辰/申亥/酉戌，复用 `_relation_utils.pair_in`）**且** 月令**本气**五行==日主之财（`ZHI_WX[月令]==WX_KE[GAN_WX[日主]]`） |
| 插入点 | `subjective/caiming.py` `assess_caiming_level` 凶向封顶链之后、tier_map 之前（函数双轨调用天然生效；凶向 severe 压贫(1) 后 cap 不抬升） |
| no-op 纪律 | tier_idx≤2 时完全不改档不追加文本（哨兵锁定：甲子/丙子/己未/甲子 谓词命中但凶向链已压小康→零文本）；文案不含「下浮封顶」（避开 rubric `_XIONG_MARKERS` 凶向直杀链——本条款是量级上限非凶向） |

### 书锚（回书逐字核行号，登记与所引偏差）

- 真阳锚① cj-教授（甲辰/丙子/己未/戊辰 乾）：`mangpai-chuji-minglixue.txt:2379-2384`「为月令的财被坐支穿…此造是制财结构，财还是可以的，教授，收入还可以」（终判批引 2375，实 2379）+`:3678-3681`「穿了月令财主日主与财星无缘…不是没钱（因甲己合，甲下坐财库）…有地位，财运可以」（终判批引 3674，实 3678）+量级佐证 `:5995`「挣工资的，也不是发大财的」（卜文另处，终判批归入 3674 锚内，登记更正）。
- 真阳锚② zj-邢铭芬（乙未/乙酉/丙戌/庚寅 坤）：`mangpai-zhongji.txt:2322-2323`「动了，但穿了财，故发不了大财，只能发点小财。如是丁酉日主见了戌则是得财之命」（终判批引 2316-2320，实 2322-2323）。
- 假阳锚（谓词全 False，哨兵锁定）：马云（酉穿戌但戌**本气**土≠丁财金——藏干口径会误中戌中气辛金；书自反例 zhongji:2323「丁酉日主见了戌则是得财之命」即马云结构，支持本气口径排除）/李嘉诚（午未不穿）/保尔森（辰戌冲非穿）/奥纳西斯（丑未冲非穿）/yx-煤矿（丑戌刑非穿）。

### 两道预注册必答题（实跑作答，探针=/tmp/t_jiaoshou_probe.py 不入仓）

1. **马云 collateral**：本气口径谓词=False（日支酉穿月令戌=True、月令戌本气=土≠丁日财=金），马云 ⚠️（引擎富/gold 巨富）不翻 ❌，famous 零回归红线不触；藏干口径对照=True（误中证明）——取本气口径依据=①三锚例月令皆本气财（子水=己财/酉金=丙财/亥水=戊财）②藏干口径 509 扫命中 14 例含马云（终判批「同构面 14 例」即藏干口径）③书自反例 zhongji:2323。
2. **§7-15 口径边界**：本条款谓词=纯结构条件（日支穿月令+月令本气财），完全不含身弱/财旺判定，与「身弱财旺 cap」（qi14/b67-森田健双锚否决，KB §5.2）机制不同族；yx-贫穷命贫困线上实测 classify_strength=从弱，命中经由=申穿亥+亥本气水=戊财（穿财结构自带双锚），预期只到 ⚠️ 非 ✅；其余否决项（方向门/mingju 宽撤/桃花/merchant/lawyer/富屋贫人 gating/中气原神收窄等）逐条核对无冲突。

### 误伤面复扫（本气口径，509 例+ famous 67+富命锚）

- 509 扫命中 **6 例**（藏干口径 14 例含马云——口径差异已登记）：3 scored 改善目标（教授/邢铭芬/贫穷命）+3 unscored（yx-书法家-2/yx-佛具/shouke-qi37-普金）；**✅误伤=0**（命中例无一 tier_static>2 且当前✅）。
- famous 67 例命中=**0**；富命锚 4 例谓词全 False。

### 翻转明细与逐条归因（全量 blind diff，vs p4）

- **trainset 财 61→63✅（+2，预警带 +1~3 内；M3：Δ+1.8%<半宽和 18.0% 噪声带内，CI 下界 44.8%→46.6%）**：cj-教授 ❌→✅（巨富→小康）/zj-邢铭芬 ⚠️→✅（富→小康，gold 平=小康）/yx-贫穷命贫困线上 ❌→⚠️（富→小康，改善非✅）——3 条全预注册。
- **heldout 三维零翻转零抖动**（官 48✅/财 47✅/职 24✅ 保）；trainset 官 102✅/职 41✅ 零翻转；M2 财命七组双集无失衡恶化（trainset 小康组 3✅→4✅、贫组 5❌→4❌，改善向）。
- **文本抖动 1 条**：yx-书法家-2 caiming_adjust 追加封顶小康文本（财维 unscored，条款文本设计内）。**偏差备案（良性）**：预注册预期抖动 3 例（含 yx-佛具/shouke-qi37-普金），实测此两例在 blind 上下文喂运岁、全量轨岁运反局先行压小康（tier_idx=2），条款 no-op 不追加文本；static 轨变档（巨富/富→小康）快照字段不收（财维 unscored），故零抖动。

### 验证（全绿）

- 哨兵 `mangpai/tests/test_t_jiaoshou_caiming.py` 6 测先红（3 红=三锚例）后绿（6/6：三锚封顶小康+马云/富命锚 guard+no-op guard）。
- pytest **1113 passed+1xf**（1107+新增 6）；verify 432+70+64+20 全绿。
- 双 seed（PYTHONHASHSEED=0 vs 默认）blind 快照剥 _meta 逐字节一致。
- regression67/famous **无变化**（马云不 REGRESSION，红线保）；calib 由 pytest 覆盖（19 测含 baseline 口径全绿）常驻 2 条（zhenbao-01 官/zhenbao-14a 财）零新增。
- check_layering+check_typing_imports 通过。

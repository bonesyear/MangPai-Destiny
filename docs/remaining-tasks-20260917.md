...
### 已关闭 · D 真实凭证冒烟（**用户决策 2026-09-18：关闭**）
**用户决策（2026-09-18）：本项关闭**（接飞书冒烟不必要）。webhook 独立部署方向**以后再看**——若彼时启用，冒烟作为该工程的一部分届时另议（参考 `kimi-remaining-plan-20260918.md` §C1 九步清单：前置=FEISHU_APP_ID/SECRET/VERIFICATION_TOKEN、**不配 Encrypt Key**（README 红线）、公网回调 URL）。**本项不再作为待办跟踪。**

...
- **第三方外发告知**：LLM 叙述通道（`subjective/llm_channel`、飞书 bot 默认开启）会把排盘数据（生辰八字、性别、出生地经度）提交至第三方大模型 **DeepSeek** API 生成叙述文本。不同意外发可设 `FEISHU_USE_LLM=0` 关闭 LLM 段，仅输出引擎直出结论。
- **免责声明**：命理分析仅供参考，不构成人生决策依据。

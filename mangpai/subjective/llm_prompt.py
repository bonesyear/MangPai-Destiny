...
def _qianyi_anchor(features: dict) -> str:
    ...
    ban = ('措辞上限「迁移/远行」，出境类断语任何形态（含否定式）一律禁止；'
           '只述象与应期，不得输出引擎特征之外的结论式建议'
           '（如「宜动不宜静」类宜忌断语，引擎无此输出=臆造）；'
           '只写象，不要声明你在遵守禁令。')
    if not markers and not moves:
        return ('【本案迁移锚定】引擎无迁移信号（qianyi 原局 marker 与应期窗均空）。'
                '迁移维必须如实说「无迁移信号」（模板语，禁止写「数据不足」字样），'
                '不得断言迁移/远行；'
...
def _xiangmao_anchor(features: dict) -> str:
    ...
    ban = ('相貌维只许引用上述 marker 描述；叙述正文任何位置不得出现'
           ...
           '锚定行含性别分流语（女看…/男看…）时，按本造性别只写对应分支，'
           '另一支不得复述（本造性别见八字行乾/坤造标记）；'
           '不要声明或解释你在遵守禁令。')
    if not parts:
        return ('【本案相貌锚定】引擎无显著相貌 marker（xiangmao 各线未命中）。'
                '相貌维 conclusion 只写「无显著相貌特征」一句'
                '（模板语，禁止写「数据不足」字样），不得给任何相貌评价，'
                '不解释、不声明禁令。')

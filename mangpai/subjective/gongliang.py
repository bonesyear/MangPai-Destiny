...
已知局限（待 P0 引擎更新：贼神捕神/净制/象法回退/领域应期）：
  ...
  导致部分源文命例功量偏低（已记入 reasons，不致误升）：
    - 入墓为功（tomb_works）：蒋介石(巳午入戌)、岳飞(未为羊刃墓)
      （李嘉诚未入辰已经库源「连墓加层」书锚路径覆盖，不在此局限）；
    - 包制（须三合成势）：克林顿(寅戌火局包制申丑，非标准三合)。
  层层相制（冲链）已经 7a 党势门补全：乾隆(子午卯酉金字塔)由 zb 党势重算冲链
  （链长≥3+冲边≥2+zb 净制）采信并采纳 zb 净制，达四层。真实制局与相生/偶然共现
  的区分、强方制弱方的势方向判定，仍依赖贼神捕神(党势-孤立目标)模块，本模块
  不臆断，以保守启发式（强制参与位+单点位原神用神同制+冲克穿实制佐证）兜底。
  源文第6章 14 例回归：14 例层数与源文一致（普例2 经同制实制佐证细化——
  合族（六合/半合）单独不支撑同制，丑经酉丑相拱/子丑合书判相生之功——
  及日干无功弃之不看（日主自克非制）后达书层）。
...
def analyze_gongliang(
    ...
    *,
    ...
    # 实制佐证位：冲/克/穿（非辅助，双方）。段氏三分法以冲制/穿制/包制为真制，
    # 合族（六合/半合/暗合）亦可为合绊/相拱之相生结构而非制——源文普通例二：
    # 丑经酉丑相拱（半合成金局，生扶丑中辛金七杀）、子丑合（合绊），书判
    # 「相生之功」非制局。同制为 +2 核心铁律，保守须冲/克/穿实制佐证，
    # 合族单独不支撑同制（正当同制案例俱有实制：蒋介石亥巳亥冲、李嘉诚亥亥克午、
    # 例八申寅申冲、奥纳西斯丑丑未冲、森田健亥戌克亥）。
    destructive_positions: Set[str] = set()
    for wa in non_aux:
        if wa.get('type') not in ('冲', '克', '穿'):
            continue
        f, t = wa.get('from_pos', ''), wa.get('to_pos', '')
        if f:
    ...
    # 单点位判定避免「财透干 + 某支藏食伤」式的偶然共现误判；实制佐证避免
    # 合族（相拱/合绊）之相生结构误认作制（普通例二丑，书判相生之功）。
    ...
    if day_wx and gans and zhis and strong_positions:
        ...
        for pos in sorted(strong_positions):  # sorted：稳定 sort 平局序，复跑确定
            if pos not in destructive_positions:
                continue  # 合族单独不支撑同制，须冲/克/穿实制佐证（普通例二丑）
            elem = _elem_of(pos, gans, zhis)
            if not elem:
                continue
    ...
    # ── 7''. 乾隆式金字塔冲链（贼神捕神党势判定补全，源文 6155-6165 乾隆例）──
    #   段氏：乾隆「做功是通过一级一级相制形成金字塔形的权力结构。子制午、午制酉、
    #   酉制卯」。冲为互向、其势方向须党势判定（本模块 _chain_length 仅认有向克链
    #   之已知局限——乾隆仅得 chain=1），故金字塔冲链以 zb 党势重算为准：
    #   链长≥3（覆盖全局四支）且以冲为骨（冲边≥2，段氏「冲为层层相制之骨」）、
    #   党势判净制（捕神成势+贼神孤立）-> 金字塔成立，+1层。
    #   门径实测全库仅乾隆/链同构且已达四层者命中；单冲边之链（普通4式）不入。
    #   7' 包制路径已计冲链者、本模块克链已计者，不重计。
    _zb_pyramid = False
    if (not _zb_bao_counted and chain_len < 2
            and _zb_clian and _zb_clian.get('detected')
            and _zb_clian.get('length', 0) >= 3
            and sum(1 for _lt, _la, _lb in _zb_clian.get('links', [])
                    if _lt == '冲') >= 2
            and _zb_jing == '净'):
        points += 1
        _zb_boost += 1
        _zb_pyramid = True
        reasons.append(
            f'层层相制：乾隆式金字塔冲链（{"->".join(_zb_clian.get("nodes", []))}，'
            f'链长{_zb_clian["length"]}、以冲为骨），zb 党势判净制佐证（+1层）'
        )
    ...
    # 包制（围制官杀）/ 金字塔冲链结构下，zb 的党势-孤立净制判定比本模块位置启发式
    # 更可靠--围制为合围式彻底之制、金字塔为逐级相制覆盖全局四支之制（段氏：制之彻底
    # 能成大富/大官），本模块「食伤透干未净制」「折扣动作」等位置缺陷信号在此结构下
    # 为伪不净（克林顿申官杀被围制入丑墓书判制之彻底；乾隆子午冲虽子为庚死地折扣，
    # 党势捕神火成势贼神金孤立，书判帝王四层）。故 bao/金字塔+zb 净双佐证时，覆本
    # 模块「不净」为「净」（仅克林顿/岳飞/乾隆具此组合，例六 zb 不成故不覆）。
    if (zhi_jing == '不净' and _zb_jing == '净' and (_zb_bao_counted or _zb_pyramid)):
        zhi_jing = '净'
        zb_jing_adopted = True
        jing_note = (jing_note + '；' if jing_note else '') + '包制围制/金字塔冲链下采信贼神捕神党势净制：制之干净'
...
def _assess_penalty(
    ...
    *,
    ...
    #   日干无功弃之不看（源文普通例三「日干无功，弃之不看，专看日支未土」）：
    #   日主自克（from_pos=day_gan 之克，我去取财之意向）非做功之制——普通例二
    #   乙克己在局而书判「相生之功」即此。故相生之功的「无制局」以实制动作
    #   （剔日主自克）为准，而非 zuogong 的制用 work_type。
    real_zhi = [wa for wa in non_aux
                if wa.get('type') in _ZHI_TYPES and not wa.get('auxiliary')
                and wa.get('from_pos') != 'day_gan']
    if wset and not real_zhi and (wset & {'生用', '化用', '禄', '合用'}):

...
- 反例 guard：坤 壬辰癸卯丙辰戊子（gaoji:14236-14252 案例八，书明写坤造，子女优）不得入损子窗。

红线：损子 desc 字面不得含「死/夭/丧」（LLM 侧另有 _scrub_death 兜底）。
岁运序列手动喂入（yingqi/liuqin 测试惯例），不改引擎岁运逻辑。

修批 E4 备案（2026-08-19，行为不变）：穿引动裁定 a·改注——实现口径注见
zinv 模块 docstring（直锚 gaoji:17465-17484 降权，F6 gaoji:14295-14312 实系
「原局有穿+岁运引动」不触发本实现）；损子「冲」增补候选收档（孤锚
gaoji:14122，gaoji:21038-21052 形态非同构，未达双锚）。本模块无穿引动哨兵
（穿引动无 A 级直锁书例可锁）。
"""
import json
...
# 脆弱对齐备案（U1 P2-1）：书到「丁为子息星」靠明文否决七杀；引擎靠 _pillar_cats
# 丢余气口径回退食伤——若藏干口径扩到余气，本节哨兵需随引擎口径重审。
...
def test_guard_an8_no_sunzi_no_jiefu():
    """坤 壬辰癸卯丙辰戊子（子女优，gaoji:14236-14252 书明写坤造）：平和岁运不得入损子窗。"""
    r = _run('壬辰癸卯丙辰戊子', '女', dys=['丙午'], lns=['庚寅'])
    assert _sunzi(r) == []

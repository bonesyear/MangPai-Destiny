...
xfail 4 条 = 引擎缺口备案（非打标错误，修法锚不足或未建模）：
  - he6 合去：书机制为「流年字（戊=巳火夫星虚透）被原局合去」，方向与
    引擎（只判原局 target 旺衰）相反，未建模。
  - he7 合绊：大运合原局（丁壬合+子丑合切断水源），非流年-局关系，
    流年语义分类器不覆盖运-局合。
  - ch4 冲去：流年冲运支触发统看（口诀「流年冲合运，十年统看」），
    统看下 ch13 干/支定向（冲动/冲去）不触发，落默认冲动；单锚不修。
  - ch8 冲破：「辰戌冲坏了夫宫」，惟戌为墓库引擎冲开规则在先；「夫宫
    无合被冲=冲破」仅此一锚，不足 ≥2 同构修法门槛。
"""
import pytest
...
class TestHeSemantics:
    ...
    @pytest.mark.xfail(reason='引擎缺口：流年字（戊=巳火夫星虚透）被原局'
                              '合去的反向语义未建模（引擎只判原局 target）')
    ...
    @pytest.mark.xfail(reason='引擎缺口：大运合原局（运-局合）非流年-局'
                              '关系，流年语义分类器不覆盖')
    def test_he7_zichou_he_ban(self):
        ...
        r = classify_he_semantic(
            ...
            dayun_zhi='丑', target_idx=2, gender='男')
        assert r['he_type'] == '合绊'
...
class TestChongSemantics:
    ...
    @pytest.mark.xfail(reason='引擎缺口：流年冲运支触发统看，统看下 ch13 '
                              '干/支定向不触发，落默认冲动；单锚不修')
    ...
    @pytest.mark.xfail(reason='引擎缺口：戌为墓库冲开规则在先；「夫宫无合'
                              '被冲=冲破」仅此一锚，不足 ≥2 同构修法门槛')

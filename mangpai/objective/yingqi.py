...
三种机制：
  1. 大限映射：四柱对应人生时段，纯位置-时段映射，与书中大限一致：
     ...
     ⚠ 宫位年龄统一决定：本映射采用书中大限值（1-18/18-35/35-55/55+），
       与 xiangfa.py 的 GONG_WEI_XIANG（1-15/16-30/31-50/50+，宫位六亲语境）
       并存不冲突——后者主六亲时限取象、前者主应期大限，分属不同理论维度，
       已在 MODULE_ATTRS.md 标注此统一决定。
...
已知争议：大限年龄区间各师有 1-15/16-30/... 与 1-18/18-35/... 两套，
          本模块依书中大限取后者，应期专用；六亲宫位取象仍用前者。
置信度：中
"""
...
# 注意此为「应期大限」专用，与 xiangfa.GONG_WEI_XIANG 的六亲宫位年龄（1-15/...）
# 分属不同维度，并存不替换（见模块 docstring 与 MODULE_ATTRS.md 统一决定）。

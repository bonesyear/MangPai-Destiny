...
| `body_parts.py` | 中性数据（盲派取用） | 干支身体部位映射（高级篇 ch11.2 主表：干主外/支主内 + ch4/中级扩展层 + 宫位身段 + 五行病机），纯数据查表零判断；为身体部位唯一事实源，xiangfa 'body' 字段保持速记简版不回写 |
...
- **宫位年龄区间统一决定（2026-07 收口）**：引擎内曾并存两套宫位年龄（高级篇审计冲突③），**已统一为大限套**：
  - 统一口径（全引擎唯一）：年柱 1-18 / 月柱 18-35 / 日柱 35-55 / 时柱 55+（段氏《盲派中级命理学》第二章应期篇大限值）。
  - `yingqi.py` 的 `DAXIAN_MAP`（应期大限）与 `xiangfa.py` 的 `GONG_WEI_XIANG`（六亲宫位取象）同采本套；原六亲口径（1-15/16-30/31-50/50+）**已废弃**。
  - 两套语义（应期兑现时段 vs 六亲时限取象）仍在各自模块使用，但年龄数字全引擎唯一，不再分歧。

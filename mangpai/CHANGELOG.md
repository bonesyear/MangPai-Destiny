...
## 2026-09-18 T-教授 · 财命窄条款微批（caiming 消费侧单点，objective/总线零改动；预注册先行）

| 项目 | 内容 |
|------|------|
| 条款 | **日支穿月令财 → 封顶小康**（`subjective/caiming.py` `assess_caiming_level` 凶向封顶链后、tier_map 前）：谓词=`pair_in(日支, 月令, LIU_HAI)` 且月令**本气**五行==日主之财（ZHI_WX/WX_KE 口径）；仅 tier_idx>2 时置 2 并追加 adjust 文本，tier_idx≤2 完全 no-op（零文本抖动）；文案不含「下浮封顶」（避 rubric 凶向直杀链）。函数双轨调用天然生效（原局轨+全量轨） |
| 书锚 | 真阳=cj-教授（chuji:2379-2384「为月令的财被坐支穿…财还是可以的，教授，收入还可以」+chuji:3678-3681「穿了月令财主日主与财星无缘」+chuji:5995「挣工资的，也不是发大财的」）+zj-邢铭芬（zhongji:2322-2323「穿了财，故发不了大财，只能发点小财」）；假阳=马云（酉穿戌但戌本气土≠丁财金，书自反例 zhongji:2323「丁酉日主见了戌则是得财之命」支持本气口径排除藏干辛金误中）/李嘉诚/保尔森/奥纳西斯/煤矿（谓词全 False，哨兵锁定） |
| 必答题 | ①马云探针实跑：本气口径 False/藏干口径 True（误中证明）→取本气口径，famous 零回归红线不触；②§7-15 核对：谓词纯结构条件不含身弱/财旺判定，与「身弱财旺 cap」双锚否决记录无交集（yx-贫穷命贫困线上实测从弱，命中经由穿财结构自带双锚，只到 ⚠️） |
| 翻转明细（全预注册） | trainset 财 **61→63✅**（+2，预警带 +1~3 内，M3 噪声带内，CI 下界 44.8%→46.6%）：cj-教授 ❌→✅（巨富→小康）/zj-邢铭芬 ⚠️→✅（富→小康）/yx-贫穷命贫困线上 ❌→⚠️（改善非✅）；**官 102✅/职 41✅ 零翻转**；heldout 三维**零翻转零抖动**（官 48✅/财 47✅/职 24✅ 保）；文本抖动 1 条（yx-书法家-2 caiming_adjust 条款文本，设计内；佛具/普金全量轨岁运反局先行压档条款 no-op=良性偏差备案） |
| 验证 | 哨兵 `test_t_jiaoshou_caiming.py` 6 测先红（3）后绿；pytest **1113 passed+1xf**；verify 432+70+64+20；双 seed 剥 _meta 逐字节一致；67/famous 无变化（马云不回归）；calib 常驻 2 条零新增（pytest 19 测全绿）；check_layering+check_typing_imports 通过；3.11.15/3.14.4 双版本 blind 冒烟剥 _meta 逐字节一致（blind 链路不触 sxtwl）。换基线 **`snapshots/20260918_t1.json`** + LATEST 推进；预注册=`docs/kimi-t-jiaoshou-caiming-prereg-20260918.md`，详账=backlog T-教授节 |

## 2026-09-18 P4 · 职业军警新面批（引擎精度批 P 系列第四批=收官批·判定改动批；预注册先行；objective muku 纯增量+zhiye military 窄消费）

| 项目 | 内容 |
|------|------|
| 检测面（objective 纯增量） | `muku.py` 新增 `detect_ku_zhi_ku(zhis)`：库制库=阳库（辰戌，阳土）收/刑 阴库（丑未，阴土），阳库恒为制方（阳制阴）；收=is_entomb 四库之土入辰墓（收式唯辰，戌论冲开不入墓），刑=丑戌/戌未；阳库冲阴库十二支不存在，kind∈{收,刑} 完备。**analyze_muku/is_entomb 等既有输出零改动**（muku 13 消费方契约不变，F2 先例全量回归=六件套实证零回归） |
| 消费条款（zhiye military，贵气门**外**） | `_score_military` F15 块后：**库制库·阳制阴（墓用执法象）+6**，要件与门=① detect_ku_zhi_ku 命中 ② 阴库成双多见（丑≥2 或未≥2，墓用结构条件2「有物可墓…成势、多见」gaoji:2190-2194+案例三双丑/例四双未明文）。墓用结构=格局级做功（gaoji:2177-2182）不走贵气门（:11956 管 8.2 字级组合）；mingju_xiong 军警 gating 照旧撤分。书锚 gaoji:2401-2417「阳库收阴库…阳制阴…实际为警察」+:11747-11756「戌未相刑，刑开官杀库…入兵营掌权之象」+:11630 丑=阴库公安象 |
| 翻转明细（全预注册） | trainset 职 **40→41✅**（+1：gj-警察墓库 merchant 7→military 7 tie_pri 胜出；⚠️ 预警带 +2~4 未达=偏差备案，库制库书锚仅 2 例同构，不为追指标放宽条款）；**官 102✅/财 61✅ 零翻转**；heldout 三维**零翻转零抖动**（官 48✅/财 47✅/职 24✅ 保）；**文本抖动 0**；命中面 12 例归因=数亿坐牢/qi22 无职 verdict 快照不收、足球/建筑-2/8721/qi50 分数升 primary 不变、li139/qi05/li141 gating 撤分（li141=岁运反局流年破坏功神） |
| 军警探针 | 8.2 书例探针 **3/10→4/10**：军官例四（戌未刑开杀库，mil 1→7 过阈）归位 military——F15 贵气门备案由墓用结构通道承接（旧哨兵「无 8.2/戌武库 evidence」仍成立）；例二（margin 6）/例九（margin 1）锚条款结构性不命中逐字不动；yx-科级 collateral 不复现（阴库 0） |
| 收档 | 8.2 例三/五/六/七/八/十+军警备案簇余（岳飞/戴笠/reg67-公安/yx-公安）+「库藏何物」分级加权（两锚各执无第三锚）——理由=backlog P4 节/预注册 §七 |
| 验证 | 哨兵 `test_p4_zhiye_kuzhiku.py` 10 测先红后绿；pytest **1107 passed+1xf**；verify 432+70+64+20；双 seed 逐字节一致；67/famous 无变化；calib 常驻 2 条零新增；M2 财命七组双集逐字不动；check_layering+check_typing_imports 通过；3.11/3.14 冒烟逐字节一致。换基线 **`snapshots/20260918_p4.json`** + LATEST 推进；预注册=`docs/kimi-p4-zhiye-muku-prereg-20260918.md`，详账=backlog P4 节。**P 系列收官**：P1-P4 四批全落地（trainset 官 96→102✅/财 59→61✅/职 40→41✅，heldout 三维四批贯穿零回退） |
...
| 项目 | 内容 |
|------|------|
| A12 女命夫宫域分流（新面） | `guanming.classify_guanming_combo` 增 `gender` 形参（缺省 None=零行为变化，calib/旧调用不动）：女命（女/坤/female）日支（夫宫）参与做功的官杀类 combo（伤食制官杀/劫刃制官杀/官杀制比劫/官杀制伤食）归「夫荣」域不计己官；印类 combo/财域劫刃制财/藏杀被制/G9/食合官支/贼捕/生用化用一律不动。书锚 chuji:2206-2209「此造女命不为当官，因夫宫巳火做功…如果是男命就为官了」+yanjiu:5646 同造重出。`analyze_guanming` 增 gender kwarg 透传，`engine.py` 调用补 gender（编排侧接线） |
| A17 旺杀入墓墓不开不作功（veto flag，仿 G6 形态） | 官杀主气地支≥2（多而墓）且全部入同一在局墓库、墓无冲刑未开（`analyze_muku` 非开库）、且无官杀做功 combo（含藏杀被制）亦无印化官杀 → 不立官命；detail 仅决定性时追加（已 False 例零文本变化）。豁免=有官杀做功（曾国藩「功在墓杀」/阎百川「旺杀入墓丑统七杀」lixiangxue:7182 = 墓统杀为所用 vs 「墓不开不作功」的书内区分本身）。书锚 chuji:1405/1409+第二独立锚 chuji:3161「墓又不开，不作功，穷命、无官」 |
| 收档 5 条（不机制化，理由入预注册 §三） | A13 争合官无力（单例孤锚+须撤 3 硬制 combo 机制不符+与 R3GUAN「合身肯定是官」冲突）/A14 合绊无功（须否决印类 combo 触方向门 10 锚禁令）/A15 制不尽→实测=合用官被穿破（印化官杀独立立官，撤 combo 不足翻判）/A16 禄上坐官（孤例+须动印类 combo）/A18 制财尽（与 G7 窄豁免锚 cj-县长同构不可分） |
| 翻转明细（全预注册） | trainset 官 **100→102✅**（+2，方案带 +2~5 下沿；M3 噪声带内，CI 下界 79.6%→81.6%）：cj-2206（A12）/cj-1395（A17）；**财/职零翻转**；heldout 三维**零翻转**（官 48✅/财 47✅/职 24✅ 保）；文本抖动 2 条（cj-妓女/shouke-qi23-闹婚不离 veto_reasons 岁运反局剥除连锁——A12 夫宫分流后官命域 veto 链收窄不再适用，机制设计内，两例官命维 unscorable 零评分影响） |
| 验证 | 哨兵 `test_p2_guanming_fp.py` 10 测先红（5 红）后绿；pytest **1087 passed+1xf**；verify 432+70+64+20；双 seed 逐字节一致（同 note 复跑 cmp 一致）；67/famous 无变化；calib 常驻 2 条零新增；四保护锚（cj-2097/yx-部长/朱元璋/reg67-公安+cj-公安附验）margin 检验全保+A4 印类方向门 10 锚复验全 True；check_layering+check_typing_imports 通过；3.11/3.14 冒烟一致。换基线 **`snapshots/20260918_p2.json`** + LATEST 推进；预注册=`docs/kimi-p2-guanming-prereg-20260918.md`，详账=backlog P2 节 |
...
| 项目 | 内容 |
|------|------|
| F6-1 降级两维（P1） | 提示语按场景区分：死词拒出=「LLM 叙述触发安全过滤不予展示」（service.py，不写「暂不可用」），一般失败维持「暂不可用」；引擎直出补迁移/相貌两段（formatter.py `_qianyi_rows`/`_xiangmao_rows`，透传 qianyi desc 与 xiangmao 命中线 marker 原文，秀气线「漂亮」经 `_xm_sanitize` 改写入红线内措辞） |
| F6-2 误报窗+死词表（P1） | 句界符补「，；」——死断+拒答标记同逗号句不再误豁免（W1 合成例 6/10）；死词表补 离世/去世/归西/病逝+同类 过世/身故/辞世（11→18 词） |
| 复合词双轨补禁（P1） | 校验器轨：`_XIANGMAO_FORBID` 扩 标致/水灵/清秀/端庄（词级匹配无单字误报，无需排除窗）；prompt 轨：SCHEMA 相貌条款+`_xiangmao_anchor` 禁令点名五词（含甜美） |
| F6-6 顺手（P2 并入） | `_xiangmao_anchor` 禁令补「禁程度词（明显/很/强）、禁评价词（有神采/明亮/灵动/灵秀）、禁引申气质总结句（艺术气息/灵动之感）」 |
| 哨兵 | 新增 9 测先红后绿：逗号窗漏杀 3 例+豁免保住/死词近义词 7 词/复合词校验器 4 词+prompt 双轨点名/程度词禁令/降级提示语两场景/直出两维段/直出 sanitize |
| 294 例离线重扫 | r4 jsonl 按新校验器重扫：迁移红线 0、死亡红线 0 保；相貌新词表抓「清秀」47 例（存量 LLM 输出，prompt 轨已点名收敛，复跑批次生效） |
| 六件套 | verify 432+70+64+20 全绿；pytest 831+1xf+19xp；blind vs `snapshots/20260821_n3.json` 零翻转（快照 `snapshots/20260822_g1.json`）；双 seed 逐字节一致；67/famous 无变化；calib 常驻 2 条零新增（由 pytest 覆盖） |
